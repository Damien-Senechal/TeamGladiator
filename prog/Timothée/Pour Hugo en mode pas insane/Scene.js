class Scene extends Phaser.Scene {
  ...
  preload() {
    ...
    // chargement du fond et de l'arme
    this.load.image("bg", "bg.png");
    this.load.image("BDF", "circle.png");

    // chargement sprite joueur
    this.load.spritesheet('player', 'player.png', {frameWidth: 48, frameHeight: 48});

    // chargement sprite joueur
    this.load.spritesheet('enemy', 'square.png', {frameWidth: 48, frameHeight: 48});
  }
  ...
  create() {
    ...
    this.BDF = this.physics.add.sprite(200, 100, "BDF");
    this.BDF.body.setAllowGravity(false);
    this.BDF.setDepth(1);
    this.BDF.setVisible(false);
    this.BDF.attack = 100
    this.BDF.disableBody()
    this.BDF.setScale(0.1)

    this.enemy = this.physics.add.sprite(700, 100, "enemy").setScale(2);
    ...
    this.physics.add.collider(this.BDF, this.enemy);
    ...
    this.physics.add.overlap(this.BDF, this.enemy, function (){
      me.enemy.hp -= me.BDF.attack;
    })
  }

  //Event qui permet de faire l'exploqion de magie
  onEvent()
  {
    this.BDF.disableBody()
    this.BDF.setVisible(false);
  ...
  update() {
    ...
    this.BDF.x = this.player.x;
    this.BDF.y = this.player.y;
